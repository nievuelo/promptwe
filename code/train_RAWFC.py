...
from model import ExplainFC #DistilEmbeddings

# from dataset.oracles.vocab_process import preprocess
from helpers.path_util import from_project_root, dirname

# from eval import evaluate_model
from eval_exp_fc5 import evaluate_model, loss_func,evaluate_model_test
...
ROOT_PROJ_PATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/CofCED/dataset/RAWFC"
...
LOG_FILE = pjoin(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/CofCED/dataset/logs", f"{date_str}_DOC_ExplainFC5-DistilBERT_auto_{ROOT_PROJ_PATH.split('/')[-1]}2-RAWFC.log")
...
def train_model(n_epochs=n_epochs,
    ...
    for epoch in range(n_epochs):
        ...
        gc.collect()
        #select explanations in training process
        for i, (oracle_ids, label_ids, raw_text_dict, lm_ids_dict) in enumerate(train_loader, 1):
            ...
            # # print(len(pred_ids[0])
            # print(len(pred_doc_ids[0]))
            # print("ggg")

            # print(len(pred_doc_ids[0]))
            # print(pred_ids[0].shape)
            # print(len(raw_text_dict['_SRC_TOK'][0]))
            # print(len(raw_text_dict['_SRC_TOK'][0][0]))
            # print(raw_text_dict['_SRC_TOK'][0][0])
            # print(raw_text_dict['_SRC_TOK'][0])
            # print(lm_ids_dict)
            # claim1='.'.join(raw_text_dict['_CLAIM_TOK'])
            # label1=label_ids[0].item()
        ...
        #(model, pjoin(ROOT_PROJ_PATH, val_file), saved_model, log=log, report_each_claim=REPORT_EACH_CLAIM).values()
    ...
    if test_file:
        ...
        tese_precision, test_recall, test_micf1, test_macrof, test_rouges, test_p_sent, test_r_sent, test_f1_sent = evaluate_model_test(best_model, pjoin(ROOT_PROJ_PATH, test_file), saved_model, log=log, report_each_claim=REPORT_EACH_CLAIM).values()
