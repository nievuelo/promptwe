...
def loss_func(criterion, y_pred, y_true, mask=None):
    ...
    return loss_.mean()
#top_k=4
def evaluate_model(model, data_url, bsl_model=None, criterion=None, sent_criterion=None, doc_criterion=None, log=None, top_k=4, report_each_claim=6):
    ...
    with torch.no_grad():
        ...
        for oracle_ids, label_ids, raw_text_dict, lm_ids_dict in tqdm(loader):
            ...
            #     print(raw_text_dict)
            #     print("yyyyyyyyyyyyyyyyyyyy")
            #     dim0,dim1 = oracle_ids[0].shape
            #     gold_out_sent_perclaim=[]
            #     for temp1 in range(dim0):
            #         for temp2 in range(dim1):
            #             if(int(oracle_ids[0][temp1][temp2])==1):
            #                 gold_out_sent_perclaim.append(raw_text_dict['_SRC_TOK'][0][temp1][temp2])
            #     print(gold_out_sent_perclaim)
            #
            #     print(raw_text_dict['_SRC_TOK'][0][0])
            #     print(raw_text_dict['_SRC_TOK'][0][0][1])
            #     i+=1

            # need to be recovered
            #extract explanations from human and from model

            perclaim=raw_text_dict['_CLAIM_TOK']
            preclaim_label=label_ids.item()
            dim0, dim1 = oracle_ids[0].shape
            gold_out_sent_perclaim = []
            perclaim_gold_sentence_merge=""
            for temp1 in range(dim0):
                for temp2 in range(dim1):
                    if (int(oracle_ids[0][temp1][temp2]) == 1 and lm_ids_dict['gold_doc_labels'][0][temp1]==1):
                        gold_out_sent_perclaim.append(raw_text_dict['_SRC_TOK'][0][temp1][temp2])
                        perclaim_gold_sentence_merge +=raw_text_dict['_SRC_TOK'][0][temp1][temp2]
            newdataset={}
            newdataset['claim']=perclaim
            newdataset['label']=preclaim_label
            newdataset['merge_explain']=gold_out_sent_perclaim
            newdataset['list_explain']=perclaim_gold_sentence_merge
            if bsl_model:
                ...
                try:
                    ...
                    # print(len(selected_indices_list[0]))
            ...
            # print("111111")
            # print(pred_doc_ids)
            # print(gold_doc_labels)
            # print(gold_doc_masks)
            # print("ssssss")
            ...
            for  sel_docs, sel_ids, gold_doc in zip(selected_docs, selected_ids, gold_src_sents):
                ...
                pred.append(' \n '.join(_pred))       #all the explaining sentences in one document
            # need to be recovered
            print(pred)
            print(pred[-1])
            newdataset['preclaim_extracted'] = pred[-1]
            new_data.append(newdataset)
        ...
        # need to be recovered
        with open('rawfc04withexplaintrain.json', 'w', encoding='UTF-8') as fp:
        ...
        report = metrics.classification_report(claim_true_list, claim_pred_list, digits=4)
        confusion = metrics.confusion_matrix(claim_true_list, claim_pred_list)
...
def evaluate_model_test(model, data_url, bsl_model=None, criterion=None, sent_criterion=None, doc_criterion=None, log=None,
                   top_k=4, report_each_claim=6):
    """ evaluating Fact-checking model on data_url
    ...
    if log == None:
        date_str = str(datetime.now()).split('.')[0].replace(' ', '_').replace(':', '')
        LOG_FILE = pjoin(os.path.dirname(os.path.abspath(__file__)) + "/dataset/logs", f'{date_str}_exp_fc_test.log')
        log = SimpleLogger(LOG_FILE, level='info',
    ...
    with torch.no_grad():
        ...
        for oracle_ids, label_ids, raw_text_dict, lm_ids_dict in tqdm(loader):
            ...
            if bsl_model:
                # pred_veraciy, (pred_evi_logits, sort_indices, selected_ids, evi_masks) = bsl_model.forward(oracle_ids, label_ids, lm_ids_dict)
                pred_veraciy, selected_indices_list, (
                pred_evi_logits, selected_ids, selected_sent_repr_mask, batch_sel_thresholds) = bsl_model.forward(
                    oracle_ids, label_ids, lm_ids_dict)
                pred_veraciy = torch.softmax(pred_veraciy, dim=-1)
                # raw_text_dict['_SRC_TOK'] raw_text_dict['_TGT_TOK']
                gold_src_sents = raw_text_dict[
                    '_TGT_TOK']  # [[' '.join(s) for s in sent_list] for sent_list in raw_text_dict['_TGT_TOK']]
            else:
                try:
                    # pred_veraciy, (pred_evi_logits, sort_indices, selected_ids, evi_masks) = model.forward(oracle_ids, label_ids, lm_ids_dict)
                    pred_veraciy, selected_indices_list, (
                    pred_evi_logits, selected_ids, selected_sent_repr_mask, batch_sel_thresholds) = model.forward(
                        oracle_ids, label_ids, lm_ids_dict)
                    # (batch_size, n_classes)
                    # sort_indices 排序后的TOP_N_DOC下标
                    pred_veraciy = torch.softmax(pred_veraciy, dim=-1)
            ...
            doc_loss = []
            for pred_doc, doc_label, masks in zip(pred_doc_ids, gold_doc_labels, gold_doc_masks):
                doc_loss.append(loss_func(doc_criterion, torch.FloatTensor(pred_doc).to(device),
                                          torch.FloatTensor(doc_label).to(device)))  # , mask=masks
            doc_loss = sum(doc_loss)
            ...
            # with open("explain.csv",'w',encoding='utf-8') as write_explain:
            #     writer=csv.writer(write_explain)
            #
        ...
        log.logger.info(
            f'Total Loss: {eval_loss / len(loader):.6f}, Veracity Acc: {eval_acc / len(loader):.6f}, Explain Acc: {sent_acc / len(loader):.6f}, Doc Acc: {doc_acc / len(loader):.6f}')
        ...
        sent_all_pred = pad_sequence(sent_all_pred, batch_first=True).cpu().numpy()
        log.logger.info("explanation generation result:\n %s",
                        classification_report(sent_all_true, sent_all_pred, labels=[0, 1],
                                              target_names=["class 0", "class 1"], digits=6))
...
def main():
    ...
    model_url = "/home/workstation-14/zxr/CofCED-test/Codes/LIAR-RAW_model_epoch7_pre0.279435_rec0.279435_micf0.279435_macf0.271089_rouge0.036066.pt"
    # test_url = from_project_root("dataset/pub_oracles/ruling_oracles_test_checked.tsv")
    test_url = from_project_root("dataset/LIAR-RAW/train/")
