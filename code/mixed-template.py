...
mytemplate1 = MixedTemplate(model=plm, tokenizer=tokenizer, text='{"placeholder":"text_a"} {"soft": "Question:"}  Is it real? {"mask"}.')

mytemplate = MixedTemplate( model=plm, tokenizer=tokenizer, text='{"placeholder":"text_a"} {"soft"} {"soft"} {"soft"} {"soft:Because"} {"placeholder":"text_a"} {"mask"}.')
# wrapped_example = mytemplate.wrap_one_example(dataset['train'][1])
# print(wrapped_example)
# print("00000000000000000")
# wrapped_example = mytemplate.wrap_one_example(dataset['train'][2])
# print(wrapped_example)
...
myverbalizer = ManualVerbalizer(tokenizer, num_classes=2,classes=classes,
                        label_words={
                            "fake":["fake","false","exaggeration","underestimate","ridiculous"],
                            "real":["real","true","scientific","authentic","fact","actual"]
                        })
...
for epoch in range(50):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        if i==0:
            ...
            # print(inputs.keys())
            # print(type(inputs.to_dict()))
            # print(inputs.to_dict())
            # print(type(inputs.values()))
            # print(inputs.values())
            # print(type(inputs.items()))
            # print(inputs.items())
        ...
        # if step == 200:
        #     print(train_dataloader.wrapped_dataset[0])
        #     print(train_dataloader.wrapped_dataset[1])
            # wrapped_example = mytemplate.wrap_one_example(dataset['train'][2])
            # print(wrapped_example)
        if step % 100 == 1:
    ...
    if (tp + tn + fp + fn) != 0 and (tp + fp) !=0 and (tp + fn)!=0 :
        ...
        if top_acc < accuracy:
            ...
            joblib.dump(prompt_model, dirs + '/prmptmixedmnl.pkl')
