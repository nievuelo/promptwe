...
def load_data(path,label_dic):
    ...
    # del claim[0]
    explain = train.preclaim_extracted.to_list()
    # del explain[0]
...
for name in ["rawfc0401train.json","rawfc0401val.json","rawfc0401test.json"]:
# for name in ["val.json"]:
    path=os.path.join(data_dir,name)
    print(path)
    one_raw_dataset = load_data(path,label_dict)
    name=name.replace('.json','')
    name=name.replace('rawfc0401','')
        ...
        if(len(raw_dataset[name][0][i])>400):
            dataset[name].append(
                InputExample(
                    guid = i,
                    text_a = raw_dataset[name][0][i][:400],
                    text_b = raw_dataset[name][1][i][:400],
                    label = raw_dataset[name][2][i],
...
for epoch in range(100):
    ...
    if (tp + tn + fp + fn) != 0 and (tp + fp) !=0 and (tp + fn)!=0 :
        ...
        if top_acc < acc:
            ...
            joblib.dump(prompt_model, dirs + '/newrawfcprmptmnl.pkl')
