...
def load_data(path,label_dic):
    ...
    # print(train['claim'])
    # print(train['label'])
    # print(train['preclaim_extracted'])
    ...
    # del claim[0]
    explain = train.greed_explain.to_list()
    # del explain[0]
...
print(np.array(raw_dataset["val"]).shape)
...
for name in ["train","val","test"]:
    ...
    lng = len(raw_dataset[name][1])
    for i in range(lng):
        if(len(raw_dataset[name][0][i])>800):
            dataset[name].append(
                InputExample(
                    guid = i,
                    text_a = raw_dataset[name][0][i][:800],
                    text_b = raw_dataset[name][1][i][:800],
                    label = raw_dataset[name][2][i],
...
plm,tokenizer,model_config,WrapperClass = load_plm(model_name='roberta',model_path="/home/workstation-14/zxr/classification-pandemic-prompt/plm/roberta")
...
myTemplate = ManualTemplate(
    text = '{"placeholder":"text_a"} It is {"mask"}. Because {"placeholder":"text_b"}',
    tokenizer = tokenizer,
)
...
#                         label_words=[ ["false"],["true"],["half"]])

#{"false": 0, "true": 1, "half": 2}
...
for epoch in range(100):
    ...
    if (tp + tn + fp + fn) != 0 and (tp + fp) !=0 and (tp + fn)!=0 :
        ...
        if top_acc < acc:
            ...
            joblib.dump(prompt_model, dirs + '/newliarrawprmptmnl.pkl')
